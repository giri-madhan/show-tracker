...
export default class SearchContainer extends React.Component {
    ...
    generateGenre = (ids) => {
      ...
      componentDidUpdate(prevProps, prevState){
          console.log('op:', prevState === this.state)
      }
                            ...
                            <div style={{width: '85%', height: 130, background: '#171515', display: 'flex', borderRadius: 5, boxShadow: '0 0 5px #000'}} key={i}>
                                ...
                                <div style={{width: '100%'}}>
                                    <div style={{display: 'flex', flexDirection: 'column', color: '#fff'}}>
                                        <div style={{textAlign: 'center', marginBottom: 5, background: '#86242a'}}>
                                            <span style={{fontSize: 20}}>{result.title}</span>
                                        </div>
                                        <span style={{paddingLeft: 20}}>
                                            <span style={{fontWeight: 700}}>Genres: </span> 
                                            <span style={{color: '#aaa'}}>{this.generateGenre(result.genre_ids)}</span>
                                        </span>
                                        <span style={{paddingLeft: 20, paddingTop: 10}}>
                                ...
                                <button style={{background: 'lightgreen', border: 'none', borderRadius: '0 5px 5px 0', width: 50, fontSize: 20}}>+</button>
                            </div>
                        )
                    }) : null}
                </div>
